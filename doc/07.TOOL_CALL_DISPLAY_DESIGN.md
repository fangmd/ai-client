# 工具调用消息显示设计方案

## 一、背景

OpenAI Responses API 支持内置工具（如 `web_search`、`file_search`），在 AI 处理请求时会调用这些工具。当前实现中，工具调用过程对用户不可见，只能看到最终的 AI 回复。为了提升用户体验，需要将工具调用的过程展示在界面上，让用户了解 AI 正在做什么。

### 1.1 工具调用流程

根据实际日志分析，Responses API 的工具调用生命周期如下：

```
1. response.output_item.added
   └─ item: { type: 'web_search_call', status: 'in_progress' }

2. response.web_search_call.in_progress
   └─ 工具调用准备中

3. response.web_search_call.searching
   └─ 正在搜索

4. response.web_search_call.completed
   └─ 搜索完成（但还没有详细信息）

5. response.output_item.done
   └─ item: { action: { type: 'search', query: 'weather: Beijing, China' } }
   └─ 获取完整的工具调用信息（包含查询内容）

6. response.output_item.added (type: 'message')
   └─ AI 开始输出文本响应

7. response.output_text.delta
   └─ 流式输出 AI 回复内容
```

## 二、方案设计

### 2.1 核心思路

将工具调用作为特殊类型的消息存储和展示，完整记录对话历史：

```
用户消息
  ↓
工具调用消息（进行中）
  ↓
工具调用消息（完成）
  ↓
AI 回复消息
```

### 2.2 技术方案

#### 方案对比

**方案A：工具调用作为消息类型**
- 优点：完整记录对话历史，便于查询和展示，数据结构清晰
- 缺点：需要修改较多现有代码，消息类型变复杂

**方案B：工具调用作为附加信息**
- 优点：对现有代码影响较小
- 缺点：不利于对话历史的完整性，查询不便

**采用方案：方案A（工具调用作为消息类型）**

理由：
1. 工具调用是对话流程的重要组成部分，应该完整记录
2. 便于后续功能扩展（如搜索、引用等）
3. 数据结构更清晰，易于理解和维护
4. 符合"数据完整性优先"的原则

## 三、技术实现

### 3.1 类型定义扩展

#### 3.1.1 Message 类型扩展

```typescript
// src/types/chat-type.ts

// 扩展消息角色
export type MessageRole = 'user' | 'assistant' | 'system' | 'tool'

// 消息内容类型
export type MessageContentType = 'text' | 'tool_call'

// 工具类型
export type ToolType = 'web_search' | 'file_search'

// 工具调用状态
export type ToolCallStatus = 'in_progress' | 'searching' | 'completed' | 'failed'

// 工具调用信息
export interface ToolCallInfo {
  itemId: string          // 工具调用的唯一标识，如 ws_05a3ccd3...
  type: ToolType          // 工具类型
  status: ToolCallStatus  // 当前状态
  query?: string          // 搜索查询内容（完成时才有）
  outputIndex?: number    // 在输出中的索引位置
  timestamp?: number      // 时间戳
}

// 扩展 Message 接口
export interface Message {
  id: bigint
  sessionId: bigint
  role: MessageRole           // 新增 'tool' 角色
  content: string
  status: 'pending' | 'sending' | 'sent' | 'error'
  totalTokens?: number
  timestamp: number
  attachments?: Attachment[]
  
  // 新增字段
  contentType?: MessageContentType  // 内容类型，默认 'text'
  toolCall?: ToolCallInfo          // 工具调用信息（仅当 contentType 为 'tool_call' 时）
}
```

#### 3.1.2 回调接口扩展

```typescript
// src/main/providers/index.ts

export interface StreamCallbacks {
  onChunk: (chunk: string) => void
  
  // 新增工具调用回调
  onToolCallStart?: (toolInfo: ToolCallInfo) => void      // 工具调用开始
  onToolCallProgress?: (toolInfo: ToolCallInfo) => void   // 工具调用进度
  onToolCallComplete?: (toolInfo: ToolCallInfo) => void   // 工具调用完成
  
  onDone: () => void
  onError: (error: Error) => void
}
```

### 3.2 数据库 Schema 扩展

```prisma
// prisma/schema.prisma

model Message {
  id          BigInt   @id
  sessionId   BigInt
  role        String   // user | assistant | system | tool
  content     String
  status      String   // pending | sending | sent | error
  totalTokens Int?
  createdAt   DateTime @default(now())
  
  // 新增字段
  contentType     String?  @default("text")  // text | tool_call
  toolType        String?  // web_search | file_search
  toolStatus      String?  // in_progress | searching | completed | failed
  toolQuery       String?  // 工具查询内容
  toolItemId      String?  // 工具调用的 item_id
  toolOutputIndex Int?     // 工具在输出中的索引
  
  session     ChatSession @relation(fields: [sessionId], references: [id], onDelete: Cascade)
  
  @@index([sessionId])
  @@index([createdAt])
  @@index([contentType])  // 新增索引，便于查询工具消息
  @@index([toolItemId])   // 新增索引，便于通过 itemId 查询
}
```

### 3.3 Provider 层实现

#### 3.3.1 事件监听和回调

```typescript
// src/main/providers/openai-provider.ts

private async streamChatWithResponsesAPI(
  messages: Omit<Message, 'id' | 'timestamp'>[],
  config: AIConfig,
  callbacks: {
    onChunk: (chunk: string) => void
    onToolCallStart?: (toolInfo: ToolCallInfo) => void
    onToolCallProgress?: (toolInfo: ToolCallInfo) => void
    onToolCallComplete?: (toolInfo: ToolCallInfo) => void
    onDone: () => void
    onError: (error: Error) => void
  },
  abortSignal?: AbortSignal,
  tools: ToolType[] = []
): Promise<void> {
  // ... 前置代码 ...

  // 用于跟踪工具调用状态
  const toolCallsMap = new Map<string, ToolCallInfo>()
  
  // 处理流式响应
  for await (const chunk of stream) {
    if (abortSignal?.aborted) {
      return
    }

    const chunkType = (chunk as any).type

    switch (chunkType) {
      // 1. 工具调用开始
      case 'response.output_item.added': {
        const event = chunk as any
        if (event.item?.type === 'web_search_call') {
          const toolInfo: ToolCallInfo = {
            itemId: event.item.id,
            type: 'web_search',
            status: 'in_progress',
            outputIndex: event.output_index,
            timestamp: Date.now()
          }
          toolCallsMap.set(event.item.id, toolInfo)
          callbacks.onToolCallStart?.(toolInfo)
          
          logDebug('Tool call started', toolInfo)
        }
        break
      }

      // 2. 工具调用进行中
      case 'response.web_search_call.in_progress': {
        const event = chunk as any
        const toolInfo = toolCallsMap.get(event.item_id)
        if (toolInfo) {
          toolInfo.status = 'in_progress'
          callbacks.onToolCallProgress?.(toolInfo)
          
          logDebug('Tool call in progress', toolInfo)
        }
        break
      }

      // 3. 工具调用搜索中
      case 'response.web_search_call.searching': {
        const event = chunk as any
        const toolInfo = toolCallsMap.get(event.item_id)
        if (toolInfo) {
          toolInfo.status = 'searching'
          callbacks.onToolCallProgress?.(toolInfo)
          
          logDebug('Tool call searching', toolInfo)
        }
        break
      }

      // 4. 工具调用完成（初步）
      case 'response.web_search_call.completed': {
        const event = chunk as any
        const toolInfo = toolCallsMap.get(event.item_id)
        if (toolInfo) {
          toolInfo.status = 'completed'
          // 暂不调用 onToolCallComplete，等待 output_item.done 获取完整信息
          logDebug('Tool call completed (waiting for details)', toolInfo)
        }
        break
      }

      // 5. 输出项完成 - 获取工具调用的完整信息
      case 'response.output_item.done': {
        const event = chunk as any
        if (event.item?.type === 'web_search_call') {
          const toolInfo = toolCallsMap.get(event.item.id)
          if (toolInfo) {
            toolInfo.status = 'completed'
            toolInfo.query = event.item.action?.query
            callbacks.onToolCallComplete?.(toolInfo)
            
            logInfo('Tool call completed with details', toolInfo)
          }
        }
        break
      }

      // 6. 文本增量 - 现有逻辑
      case 'response.output_text.delta': {
        const textDeltaEvent = chunk as ResponseTextDeltaEvent
        if (textDeltaEvent.delta) {
          chunkCount++
          callbacks.onChunk(textDeltaEvent.delta)
        }
        break
      }

      // 7. 内容部分添加 - 现有逻辑
      case 'response.content_part.added': {
        const contentPartEvent = chunk as ResponseContentPartAddedEvent
        const part = contentPartEvent.part
        if (part.type === 'output_text' && 'text' in part && part.text) {
          chunkCount++
          callbacks.onChunk(part.text)
        }
        break
      }

      // 8. 响应完成
      case 'response.completed': {
        const event = chunk as any
        logInfo('Response completed', {
          totalOutputItems: event.response?.output?.length,
          usage: event.response?.usage
        })
        break
      }
    }
  }

  logInfo('OpenAI Responses API stream chat completed successfully', {
    model: config.model,
    totalChunks: chunkCount,
    totalToolCalls: toolCallsMap.size
  })
  callbacks.onDone()
}
```

### 3.4 Handler 层实现

#### 3.4.1 工具调用消息管理

```typescript
// src/main/handlers/ai-handler.ts

async function handleStreamChat(
  event: IpcMainInvokeEvent,
  params: StreamChatParams
): Promise<void> {
  // ... 前置代码 ...

  // 用于存储工具调用消息的 ID 映射
  const toolCallMessageIds = new Map<string, bigint>()

  await provider.streamChat(
    messages,
    config,
    {
      // 文本块回调 - 现有逻辑
      onChunk: (chunk: string) => {
        accumulatedContent += chunk
        event.sender.send(`ai:streamChunk:${requestId}`, {
          requestId,
          chunk
        })
      },

      // 工具调用开始 - 创建工具消息
      onToolCallStart: async (toolInfo: ToolCallInfo) => {
        try {
          const toolMessage = await messageRepository.create({
            sessionId: params.sessionId,
            role: 'tool',
            content: getToolCallStartMessage(toolInfo),
            status: 'sending',
            contentType: 'tool_call',
            toolType: toolInfo.type,
            toolStatus: toolInfo.status,
            toolItemId: toolInfo.itemId,
            toolOutputIndex: toolInfo.outputIndex
          })
          
          toolCallMessageIds.set(toolInfo.itemId, toolMessage.id)
          
          // 通知前端工具调用开始
          event.sender.send(`ai:toolCallStart:${requestId}`, {
            requestId,
            toolInfo,
            messageId: toolMessage.id,
            message: toolMessage
          })
          
          logInfo('Tool call started and message created', {
            toolInfo,
            messageId: toolMessage.id
          })
        } catch (error) {
          logError('Failed to create tool call message', error)
        }
      },

      // 工具调用进度更新
      onToolCallProgress: async (toolInfo: ToolCallInfo) => {
        const messageId = toolCallMessageIds.get(toolInfo.itemId)
        if (messageId) {
          try {
            const updatedMessage = await messageRepository.update(messageId, {
              content: getToolCallProgressMessage(toolInfo),
              toolStatus: toolInfo.status
            })
            
            // 通知前端进度更新
            event.sender.send(`ai:toolCallProgress:${requestId}`, {
              requestId,
              toolInfo,
              messageId,
              message: updatedMessage
            })
            
            logDebug('Tool call progress updated', {
              toolInfo,
              messageId
            })
          } catch (error) {
            logError('Failed to update tool call message', error)
          }
        }
      },

      // 工具调用完成
      onToolCallComplete: async (toolInfo: ToolCallInfo) => {
        const messageId = toolCallMessageIds.get(toolInfo.itemId)
        if (messageId) {
          try {
            const updatedMessage = await messageRepository.update(messageId, {
              content: getToolCallCompleteMessage(toolInfo),
              status: 'sent',
              toolStatus: toolInfo.status,
              toolQuery: toolInfo.query
            })
            
            // 通知前端工具调用完成
            event.sender.send(`ai:toolCallComplete:${requestId}`, {
              requestId,
              toolInfo,
              messageId,
              message: updatedMessage
            })
            
            logInfo('Tool call completed and message updated', {
              toolInfo,
              messageId
            })
          } catch (error) {
            logError('Failed to update tool call message', error)
          }
        }
      },

      // 完成回调 - 现有逻辑
      onDone: () => {
        event.sender.send(`ai:streamDone:${requestId}`, { requestId })
      },

      // 错误回调 - 现有逻辑
      onError: (error: Error) => {
        event.sender.send(`ai:streamError:${requestId}`, {
          requestId,
          error: error.message
        })
      }
    },
    abortSignal,
    { tools: params.tools }
  )
}

// 辅助函数：生成工具调用的消息内容
function getToolCallStartMessage(toolInfo: ToolCallInfo): string {
  switch (toolInfo.type) {
    case 'web_search':
      return '🔍 正在搜索网络...'
    case 'file_search':
      return '📁 正在搜索文件...'
    default:
      return '⚙️ 正在执行工具调用...'
  }
}

function getToolCallProgressMessage(toolInfo: ToolCallInfo): string {
  switch (toolInfo.status) {
    case 'in_progress':
      return '🔍 搜索准备中...'
    case 'searching':
      return '🔍 正在搜索...'
    default:
      return getToolCallStartMessage(toolInfo)
  }
}

function getToolCallCompleteMessage(toolInfo: ToolCallInfo): string {
  const query = toolInfo.query ? `\n查询：${toolInfo.query}` : ''
  switch (toolInfo.type) {
    case 'web_search':
      return `✅ 网络搜索完成${query}`
    case 'file_search':
      return `✅ 文件搜索完成${query}`
    default:
      return `✅ 工具调用完成${query}`
  }
}
```

### 3.5 Repository 层扩展

```typescript
// src/main/repository/message.ts

export class MessageRepository {
  // 扩展 create 方法支持工具调用字段
  async create(data: {
    sessionId: bigint
    role: string
    content: string
    status: string
    contentType?: string
    toolType?: string
    toolStatus?: string
    toolItemId?: string
    toolOutputIndex?: number
    toolQuery?: string
  }): Promise<Message> {
    const id = generateSnowflakeId()
    
    const message = await prisma.message.create({
      data: {
        id,
        sessionId: data.sessionId,
        role: data.role,
        content: data.content,
        status: data.status,
        contentType: data.contentType,
        toolType: data.toolType,
        toolStatus: data.toolStatus,
        toolItemId: data.toolItemId,
        toolOutputIndex: data.toolOutputIndex,
        toolQuery: data.toolQuery
      }
    })
    
    return this.mapToMessage(message)
  }

  // 扩展 update 方法支持工具调用字段
  async update(
    id: bigint,
    data: {
      content?: string
      status?: string
      toolStatus?: string
      toolQuery?: string
    }
  ): Promise<Message> {
    const message = await prisma.message.update({
      where: { id },
      data
    })
    
    return this.mapToMessage(message)
  }

  // 新增：通过 toolItemId 查询工具调用消息
  async findByToolItemId(toolItemId: string): Promise<Message | null> {
    const message = await prisma.message.findFirst({
      where: {
        toolItemId,
        contentType: 'tool_call'
      }
    })
    
    return message ? this.mapToMessage(message) : null
  }

  // 映射数据库记录到 Message 类型
  private mapToMessage(record: any): Message {
    return {
      id: record.id,
      sessionId: record.sessionId,
      role: record.role,
      content: record.content,
      status: record.status,
      totalTokens: record.totalTokens,
      timestamp: record.createdAt.getTime(),
      contentType: record.contentType,
      toolCall: record.contentType === 'tool_call' ? {
        itemId: record.toolItemId,
        type: record.toolType,
        status: record.toolStatus,
        query: record.toolQuery,
        outputIndex: record.toolOutputIndex
      } : undefined
    }
  }
}
```

### 3.6 IPC 通道定义

```typescript
// src/common/constants/ipc.ts

export const IPC_CHANNELS = {
  // ... 现有通道 ...
  
  // 工具调用相关
  TOOL_CALL_START: 'ai:toolCallStart',      // 工具调用开始
  TOOL_CALL_PROGRESS: 'ai:toolCallProgress', // 工具调用进度
  TOOL_CALL_COMPLETE: 'ai:toolCallComplete'  // 工具调用完成
} as const
```

### 3.7 前端界面实现

#### 3.7.1 工具调用消息组件

```typescript
// src/renderer/src/chat/tool-call-item.tsx

import { CheckCircle, XCircle } from 'lucide-react'
import { Message } from '@/types'
import { LoadingSpinner } from '@/components/loading'

interface ToolCallItemProps {
  message: Message
}

export function ToolCallItem({ message }: ToolCallItemProps) {
  const { toolCall } = message
  
  if (!toolCall) return null

  // 获取状态图标
  const getStatusIcon = () => {
    switch (toolCall.status) {
      case 'in_progress':
      case 'searching':
        return <LoadingSpinner className="w-4 h-4" />
      case 'completed':
        return <CheckCircle className="w-4 h-4 text-green-500" />
      case 'failed':
        return <XCircle className="w-4 h-4 text-red-500" />
      default:
        return null
    }
  }

  // 获取工具图标
  const getToolIcon = () => {
    switch (toolCall.type) {
      case 'web_search':
        return '🔍'
      case 'file_search':
        return '📁'
      default:
        return '⚙️'
    }
  }

  // 获取工具名称
  const getToolName = () => {
    switch (toolCall.type) {
      case 'web_search':
        return '网络搜索'
      case 'file_search':
        return '文件搜索'
      default:
        return '工具调用'
    }
  }

  // 获取状态文本
  const getStatusText = () => {
    switch (toolCall.status) {
      case 'in_progress':
        return '准备中...'
      case 'searching':
        return '搜索中...'
      case 'completed':
        return '已完成'
      case 'failed':
        return '失败'
      default:
        return ''
    }
  }

  return (
    <div className="tool-call-item bg-gray-50 dark:bg-gray-800 rounded-lg p-4 my-2 border border-gray-200 dark:border-gray-700 transition-all">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-lg">{getToolIcon()}</span>
        <span className="font-medium text-gray-900 dark:text-gray-100">
          {getToolName()}
        </span>
        <div className="flex items-center gap-1.5 ml-auto">
          {getStatusIcon()}
          <span className="text-sm text-gray-500 dark:text-gray-400">
            {getStatusText()}
          </span>
        </div>
      </div>
      
      {toolCall.query && (
        <div className="mt-2 text-sm text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-900 rounded p-2">
          <span className="font-medium">查询：</span>
          <span className="ml-1">{toolCall.query}</span>
        </div>
      )}
    </div>
  )
}
```

#### 3.7.2 消息列表组件更新

```typescript
// src/renderer/src/chat/message-item.tsx

export function MessageItem({ message }: MessageItemProps) {
  // 工具调用消息
  if (message.contentType === 'tool_call') {
    return <ToolCallItem message={message} />
  }

  // 普通消息 - 现有逻辑
  return (
    <div className={cn(
      'message-item',
      message.role === 'user' ? 'user-message' : 'assistant-message'
    )}>
      {/* ... 现有消息展示逻辑 ... */}
    </div>
  )
}
```

#### 3.7.3 聊天组件监听工具调用事件

```typescript
// src/renderer/src/page/chat.tsx

useEffect(() => {
  // ... 现有监听逻辑 ...

  // 监听工具调用开始
  const handleToolCallStart = (data: any) => {
    const { requestId, message } = data
    if (requestId === currentRequestId) {
      // 添加工具调用消息到消息列表
      chatStore.addMessage(message)
    }
  }

  // 监听工具调用进度
  const handleToolCallProgress = (data: any) => {
    const { requestId, message } = data
    if (requestId === currentRequestId) {
      // 更新工具调用消息
      chatStore.updateMessage(message.id, message)
    }
  }

  // 监听工具调用完成
  const handleToolCallComplete = (data: any) => {
    const { requestId, message } = data
    if (requestId === currentRequestId) {
      // 更新工具调用消息
      chatStore.updateMessage(message.id, message)
    }
  }

  window.electron.ipcRenderer.on('ai:toolCallStart', handleToolCallStart)
  window.electron.ipcRenderer.on('ai:toolCallProgress', handleToolCallProgress)
  window.electron.ipcRenderer.on('ai:toolCallComplete', handleToolCallComplete)

  return () => {
    window.electron.ipcRenderer.removeListener('ai:toolCallStart', handleToolCallStart)
    window.electron.ipcRenderer.removeListener('ai:toolCallProgress', handleToolCallProgress)
    window.electron.ipcRenderer.removeListener('ai:toolCallComplete', handleToolCallComplete)
  }
}, [currentRequestId])
```

## 四、实现步骤

### 阶段一：基础工具调用显示（核心功能）

- [x] **步骤1：数据库 Schema 扩展**
  - [x] 1.1 添加工具调用相关字段到 Message 表
  - [x] 1.2 创建数据库迁移
  - [x] 1.3 执行迁移并验证

- [x] **步骤2：类型定义扩展**
  - [x] 2.1 扩展 `MessageRole` 类型，添加 `tool` 角色
  - [x] 2.2 定义 `ToolCallInfo` 接口
  - [x] 2.3 扩展 `Message` 接口，添加 `contentType` 和 `toolCall` 字段
  - [x] 2.4 扩展 `StreamCallbacks` 接口，添加工具调用回调

- [x] **步骤3：Provider 层实现**
  - [x] 3.1 在 `streamChatWithResponsesAPI` 中添加工具调用事件监听
  - [x] 3.2 实现 `response.output_item.added` 事件处理（工具调用开始）
  - [x] 3.3 实现 `response.output_item.done` 事件处理（工具调用完成）
  - [x] 3.4 调用工具回调函数

- [x] **步骤4：Handler 层实现**
  - [x] 4.1 实现 `onToolCallStart` 回调，创建工具消息
  - [x] 4.2 实现 `onToolCallComplete` 回调，更新工具消息
  - [x] 4.3 添加工具消息 ID 映射管理
  - [x] 4.4 实现辅助函数生成工具消息内容

- [x] **步骤5：Repository 层扩展**
  - [x] 5.1 扩展 `create` 方法支持工具调用字段
  - [x] 5.2 扩展 `update` 方法支持工具调用字段
  - [x] 5.3 扩展 `mapToMessage` 方法处理工具调用数据

### 阶段二：实时状态更新（进度显示）

- [x] **步骤6：Provider 层增强**
  - [x] 6.1 实现 `response.web_search_call.in_progress` 事件处理
  - [x] 6.2 实现 `response.web_search_call.searching` 事件处理
  - [x] 6.3 调用 `onToolCallProgress` 回调

- [x] **步骤7：Handler 层增强**
  - [x] 7.1 实现 `onToolCallProgress` 回调
  - [x] 7.2 实时更新数据库中的工具消息状态
  - [x] 7.3 通过 IPC 发送进度更新事件

- [x] **步骤8：IPC 通道定义**
  - [x] 8.1 添加 `ai:toolCallStart` 通道
  - [x] 8.2 添加 `ai:toolCallProgress` 通道
  - [x] 8.3 添加 `ai:toolCallComplete` 通道

### 阶段三：UI 实现和优化

- [x] **步骤9：前端组件实现**
  - [x] 9.1 创建 `ToolCallItem` 组件
  - [x] 9.2 实现工具图标和状态图标
  - [x] 9.3 实现查询内容展示
  - [x] 9.4 添加加载动画

- [x] **步骤10：消息列表集成**
  - [x] 10.1 在 `MessageItem` 中判断消息类型
  - [x] 10.2 渲染工具调用组件
  - [x] 10.3 处理工具消息的时间戳显示

- [x] **步骤11：聊天页面监听**
  - [x] 11.1 监听 `ai:toolCallStart` 事件
  - [x] 11.2 监听 `ai:toolCallProgress` 事件
  - [x] 11.3 监听 `ai:toolCallComplete` 事件
  - [x] 11.4 更新 chatStore 状态

- [x] **步骤12：样式优化**
  - [x] 12.1 设计工具调用消息的视觉样式
  - [x] 12.2 添加状态过渡动画
  - [x] 12.3 适配暗色模式
  - [x] 12.4 优化移动端显示

### 阶段四：测试和完善

- [x] **步骤13：功能测试**
  - [x] 13.1 测试工具调用消息创建
  - [x] 13.2 测试状态更新流程
  - [x] 13.3 测试多个工具调用的情况
  - [x] 13.4 测试工具调用失败的情况

- [x] **步骤14：边界情况处理**
  - [x] 14.1 处理网络中断时的工具调用状态
  - [x] 14.2 处理用户取消时的工具调用清理
  - [x] 14.3 处理工具调用超时
  - [x] 14.4 添加错误提示

- [x] **步骤15：性能优化**
  - [x] 15.1 优化工具消息更新频率
  - [x] 15.2 避免不必要的数据库写入
  - [x] 15.3 优化前端渲染性能

## 五、界面设计

### 5.1 工具调用消息展示样式

```
┌─────────────────────────────────────────┐
│  用户消息                                │
│  今天北京天气怎么样？                    │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│  🔍 网络搜索              ⏳ 搜索中...   │
│  ┌───────────────────────────────────┐  │
│  │ 查询：weather: Beijing, China     │  │
│  └───────────────────────────────────┘  │
└─────────────────────────────────────────┘
         ↓ （状态更新）
┌─────────────────────────────────────────┐
│  🔍 网络搜索              ✅ 已完成      │
│  ┌───────────────────────────────────┐  │
│  │ 查询：weather: Beijing, China     │  │
│  └───────────────────────────────────┘  │
└─────────────────────────────────────────┘

┌─────────────────────────────────────────┐
│  AI 助手                                 │
│  北京目前晴，气温约 **2°C（36°F）**...   │
└─────────────────────────────────────────┘
```

### 5.2 状态图标

- **进行中/搜索中**：旋转的加载图标 ⏳
- **已完成**：绿色对勾 ✅
- **失败**：红色叉号 ❌

### 5.3 颜色方案

- **背景色（浅色模式）**：`bg-gray-50` + `border-gray-200`
- **背景色（暗色模式）**：`bg-gray-800` + `border-gray-700`
- **查询内容背景**：`bg-white` / `bg-gray-900`
- **成功状态**：绿色 `text-green-500`
- **失败状态**：红色 `text-red-500`

## 六、数据流设计

### 6.1 工具调用创建流程

```
OpenAI Responses API
  ↓ response.output_item.added
Provider: onToolCallStart()
  ↓
Handler: 创建工具消息到数据库
  ↓
Handler: 发送 IPC 事件
  ↓
Renderer: 添加消息到 chatStore
  ↓
UI: 渲染工具调用组件（进行中状态）
```

### 6.2 工具调用更新流程

```
OpenAI Responses API
  ↓ response.web_search_call.searching
Provider: onToolCallProgress()
  ↓
Handler: 更新数据库中的消息状态
  ↓
Handler: 发送 IPC 事件
  ↓
Renderer: 更新 chatStore 中的消息
  ↓
UI: 更新工具调用组件状态
```

### 6.3 工具调用完成流程

```
OpenAI Responses API
  ↓ response.output_item.done
Provider: onToolCallComplete()
  ↓
Handler: 更新数据库（状态+查询内容）
  ↓
Handler: 发送 IPC 事件
  ↓
Renderer: 更新 chatStore 中的消息
  ↓
UI: 渲染完成状态（显示查询内容）
```

## 七、注意事项

### 7.1 性能考虑

1. **数据库写入优化**
   - 工具调用状态更新频繁，考虑批量更新或防抖
   - 只在关键状态变化时写入数据库（开始、完成、失败）
   - 中间状态（in_progress, searching）可以只更新前端，不写数据库

2. **前端渲染优化**
   - 使用 React.memo 优化工具调用组件
   - 避免不必要的重渲染
   - 使用虚拟滚动处理大量消息

### 7.2 错误处理

1. **工具调用失败**
   - 捕获工具调用错误事件
   - 更新消息状态为 `failed`
   - 显示友好的错误提示

2. **网络中断**
   - 保留工具调用消息的最后状态
   - 提供重试机制（如果适用）

3. **用户取消**
   - 清理未完成的工具调用消息
   - 或标记为已取消状态

### 7.3 向后兼容

1. **现有消息**
   - `contentType` 默认为 `text`
   - 现有消息无需迁移，继续正常显示

2. **旧版本数据**
   - 新增字段全部可空
   - 不影响现有数据读取

### 7.4 扩展性

1. **支持更多工具类型**
   - 设计灵活的 `ToolType` 类型
   - 工具图标和样式可配置

2. **工具结果展示**
   - 预留扩展接口显示工具返回的详细结果
   - 如搜索结果、文件片段等

## 八、后续优化

### 8.1 功能增强

1. **工具调用结果展示**
   - 显示 web_search 的搜索结果摘要
   - 显示引用来源链接
   - 支持点击查看完整结果

2. **工具调用历史**
   - 支持查看历史工具调用记录
   - 支持导出工具调用数据

3. **工具调用统计**
   - 统计工具使用频率
   - 分析工具调用耗时

### 8.2 用户体验优化

1. **加载动画**
   - 更丰富的加载动画效果
   - 状态过渡动画

2. **交互优化**
   - 支持折叠/展开工具调用详情
   - 支持复制查询内容

3. **通知提示**
   - 工具调用完成时的通知
   - 工具调用失败时的提示

## 九、参考资料

### 9.1 相关文档

- [06.OPENAI_RESPONSES_API_DESIGN.md](./06.OPENAI_RESPONSES_API_DESIGN.md) - Responses API 集成方案
- [00.AI_CHAT_DESIGN.md](./00.AI_CHAT_DESIGN.md) - AI 聊天功能设计
- [01.CHAT_SESSION_MANAGEMENT_DESIGN.md](./01.CHAT_SESSION_MANAGEMENT_DESIGN.md) - 会话管理设计

### 9.2 API 参考

- OpenAI Responses API 文档
- OpenAI SDK 文档

### 9.3 实现参考

- 实际日志分析（见附录）
- Responses API 事件流程

## 附录：Responses API 事件流程示例

```json
// 1. 响应创建
{
  "type": "response.created",
  "response": {
    "status": "in_progress",
    "tools": [{ "type": "web_search" }]
  }
}

// 2. 工具调用开始
{
  "type": "response.output_item.added",
  "item": {
    "id": "ws_05a3ccd3...",
    "type": "web_search_call",
    "status": "in_progress"
  }
}

// 3. 工具调用进度
{
  "type": "response.web_search_call.in_progress",
  "item_id": "ws_05a3ccd3..."
}

// 4. 工具调用搜索中
{
  "type": "response.web_search_call.searching",
  "item_id": "ws_05a3ccd3..."
}

// 5. 工具调用完成（初步）
{
  "type": "response.web_search_call.completed",
  "item_id": "ws_05a3ccd3..."
}

// 6. 工具调用完成（详细信息）
{
  "type": "response.output_item.done",
  "item": {
    "id": "ws_05a3ccd3...",
    "type": "web_search_call",
    "status": "completed",
    "action": {
      "type": "search",
      "query": "weather: Beijing, China"
    }
  }
}

// 7. AI 消息开始
{
  "type": "response.output_item.added",
  "item": {
    "type": "message",
    "role": "assistant"
  }
}

// 8. 文本内容流式输出
{
  "type": "response.output_text.delta",
  "delta": "北京目前晴..."
}

// 9. 响应完成
{
  "type": "response.completed",
  "response": {
    "status": "completed",
    "output": [
      { "type": "web_search_call", "status": "completed" },
      { "type": "message", "role": "assistant" }
    ]
  }
}
```

