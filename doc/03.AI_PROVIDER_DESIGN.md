...
export const IPC_CHANNELS = {
  aiProvider: {
    ...
    update: 'ai-provider:update',       // 更新 AI Provider
...
interface AiProviderIPC {
  ...
  update: {
    request: { id: bigint, data: UpdateAiProviderData }
    response: IPCResponse<AiProvider>
  }
  delete: {
...
| 字段         | 类型     | 添加模式 | 编辑模式 | 说明                     |
| ------------ | -------- | -------- | -------- | ------------------------ |
| name         | text     | 可选     | 可选     | 名称（用于标识）         |
| provider     | select   | 必填     | 必填     | 提供商类型               |
| apiKey       | password | 必填     | 可选     | API 密钥（编辑时留空不修改）|
| baseURL      | text     | 可选     | 可选     | 基础 URL                 |
| model        | text     | 必填     | 必填     | 模型名称                 |
| temperature  | range    | 可选     | 可选     | 温度参数（0-2）          |
| maxTokens    | number   | 可选     | 可选     | 最大 token 数            |
| organization | text     | 可选     | 可选     | OpenAI 组织 ID（条件显示）|
| isDefault    | checkbox | 可选     | 可选     | 是否设为默认             |

**模式区分**：

- **添加模式**：`editProvider` 为空，调用 `ai-provider:create` 接口
- **编辑模式**：`editProvider` 不为空，调用 `ai-provider:update` 接口，API Key 留空时不更新
...
6. ✅ 实现 delete 处理器
7. ✅ 实现 setDefault 处理器
8. ✅ 实现 AIProviderHandler.unregister()

### 阶段六：前端组件实现
1. ✅ 实现 AiModelList 组件（含编辑入口）
2. ✅ 实现 AiModelDialog 组件（支持添加/编辑模式）
...
**实现方式**：
- 复用 `AiModelDialog` 组件，通过 `editProvider` 属性区分添加/编辑模式
- 编辑模式下，API Key 字段为可选（留空表示不修改）
- 编辑时自动填充现有配置数据
