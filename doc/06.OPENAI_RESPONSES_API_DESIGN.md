...
### 步骤1：扩展类型定义
- [ ] 扩展 `streamChat` 方法签名，支持传递工具列表参数
- [ ] 无需修改 `AIConfig` 类型（API 选择自动判断）

### 步骤2：实现 API 选择逻辑
- [ ] 添加 `shouldUseResponsesAPI()` 方法判断使用哪个 API
- [ ] 根据 `model` 字段自动判断：以 `gpt` 或 `o1` 开头 → Responses API，其他 → Chat Completions API
- [ ] 判断逻辑简单：检查 model 是否以 `gpt` 或 `o1` 开头（不区分大小写）

### 步骤3：实现 Responses API 调用
- [ ] 创建 `streamChatWithResponsesAPI()` 方法
- [ ] 处理 Responses API 的流式响应
- [ ] 处理工具调用结果（如 web_search 的引用）
- [ ] 实现默认工具逻辑：OpenAI 官方模型自动启用 web_search 和 file_search
- [ ] 实现工具合并逻辑：默认工具与上层传入工具合并去重

### 步骤4：重构现有代码
- [ ] 将现有 `streamChat()` 方法重构为 `streamChatWithCompletionsAPI()`
- [ ] 在 `streamChat()` 中根据选择调用对应方法
- [ ] 保持接口兼容性

### 步骤5：测试验证
- [ ] 测试 Chat Completions API（确保现有功能正常）
- [ ] 测试 Responses API（新功能）
- [ ] 测试自动选择逻辑
- [ ] 测试工具调用（web_search）
